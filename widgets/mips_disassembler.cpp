...
void mips_Decode(unsigned int opcode, unsigned int addr, char* output, int arrows) {
	...
	const char* pri_op[] = {
		nullptr		,nullptr	,"j       "	,"jal     ",//4
		"beq     "	,"bne     "	,"blez    "	,"bgtz    ",//8
		"addi    "	,"addiu   "	,"slti    "	,"sltiu   ",//12
		"andi    "	,"ori     "	,"xori    "	,"lui     ",//16
		"cop0    "	,"cop1<!> "	,"cop2    "	,"cop3<!> ",//24
		nullptr		, nullptr	,nullptr	, nullptr,		//32
		nullptr		, nullptr	,nullptr	, nullptr,		//36
		nullptr		, nullptr	,nullptr	, nullptr,		//40
		"lb      "	,"lh      "	,"lwl     "	,"lw      ",//44
		"lbu     "	,"lhu     "	,"lwr     "	,nullptr,	//48
		"sb      "	,"sh      "	,"swl     "	,"sw      ",
		nullptr		,nullptr	,"swr     "	,nullptr,
	};
	
	const char* sc_op[] = {
		"sll     "	,nullptr	,"srl     "	,"sra     "	,	//0
		"sllv    "	,nullptr	,"srlv    "	,"srav    "	,	//4
		"jr      "	,"jalr    "	,nullptr	,nullptr	,	//8
		"syscall "	,"break   "	,nullptr	,nullptr	,	//12
		"mfhi    "	,"mthi    "	,"mflo    "	,"mtlo    "	,	//16
		nullptr		,nullptr	,nullptr	,nullptr	,	//20
		"mult    "	,"multu   "	,"div     "	,"divu    "	,	//24
		nullptr		,nullptr	,nullptr	,nullptr	,	//28
	...
	if( opcode ) {
		...
		// Primary
			...
			if( OPCODE1(opcode) == 0x1 ) {	// Branches
			...
			} else if( ( OPCODE1(opcode) >= 0x8 ) && ( OPCODE1(opcode) <= 0x9 ) ) {
			...
			} else if( ( OPCODE1(opcode) >= 0xa ) && ( OPCODE1(opcode) <= 0xe ) ) {
				
				strcpy( output, pri_op[OPCODE1(opcode)] );
				
				sprintf( temp, "%s, %s, $%X", regs[OPREGT(opcode)], 
				...
				strcpy( output, pri_op[OPCODE1(opcode)] );
				sprintf( temp, "%s, $%X", regs[OPREGT(opcode)], OPIMM16(opcode) );
				strcat( output, temp );
			
			} else if( ( OPCODE1(opcode) >= 0x20 ) && ( OPCODE1(opcode) <= 0x2e ) ) {
