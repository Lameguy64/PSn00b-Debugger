...
static const char *G_header[] = { "Address", "Comment", 0 };


Fl_Bookmark_Table::Fl_Bookmark_Table(int x, int y, int w, int h, const char *l) 
...
Fl_Bookmark_Table::~Fl_Bookmark_Table() {
}

void Fl_Bookmark_Table::sort_column(int col, int reverse) {
...
void Fl_Bookmark_Table::additem(ProjectClass::BOOKMARK *mark_item) {
	...
	sprintf( strbuff, "$%08X", mark_item->addr );
	c = strbuff;
	r.cols.push_back( c );
	r.cols.push_back( mark_item->remark );
	r.addr = mark_item->addr;
...
void Fl_Bookmark_Table::draw_sort_arrow(int X,int Y,int W,int H) {
...
void Fl_Bookmark_Table::draw_cell(TableContext context, int R, int C, int X, int Y, int W, int H) {
...
unsigned int Fl_Bookmark_Table::item(int row) {
	
	if( ( row < 0 ) || ( row >= rows() ) ) {
		return 0;
	}
	
	return _row_items[row].addr;
	
}

void Fl_Bookmark_Table::autowidth(int pad) {
...
int Fl_Bookmark_Table::handle(int event) {
