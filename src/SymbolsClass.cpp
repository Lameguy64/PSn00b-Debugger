...
const char* SymbolsClass::CorrectSymbol(const char *name) {
	
	for(int i=symbols.size()-1; i>=0; i--) {
		
		if( strcasecmp(name, symbols[i].name.c_str()) == 0 ) {
			return symbols[i].name.c_str();
		}
...
unsigned int SymbolsClass::ResolveSymbol(const char *name, int &found) {
